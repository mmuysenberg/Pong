...
public class Box implements Serializable {
	...
	public void update() {
		...
		{
			if(ballLoc.x >= paddleLoc[0].x - ballRadius-paddleHeight*2 && ballLoc.y >= paddleLoc[0].y && // if ball y is paddle yish
...
//					ballLoc.y <= paddleLoc[0].y + paddleWidth) {
		...
		//if(ballLoc.x - ballRadius <= boxUpperLeft.x - paddleHeight) { // test tomorrow
			if (ballLoc.x <= paddleLoc[1].x + ballRadius+paddleHeight*2 && ballLoc.y >= paddleLoc[1].y && ballLoc.y <= paddleLoc[1].y + paddleWidth) {
				if(yAngle == -1)
					ballVy = 16;
				else if(yAngle == 1)
					ballVy = -16;
